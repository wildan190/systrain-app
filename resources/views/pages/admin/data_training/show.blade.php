<x-layouts.app title="Data Training - Detail Peserta {{ $detailPesertaId }}">
    <div class="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center p-6">
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 w-full max-w-4xl transition-all">
            <h2 class="text-3xl font-bold text-center mb-6 text-gray-800 dark:text-gray-200">
                Data Training - Detail Peserta {{ $detailPesertaId }}
            </h2>

            <!-- Flash Message -->
            <div id="toast-container" class="fixed top-4 right-4 z-50"></div>

            <form id="uploadForm" action="{{ route('data_training.store', $detailPesertaId) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @php
                        $fields = [
                            'upload_pas_foto_biru' => 'Pas Foto Background Biru',
                            'upload_ktp' => 'KTP',
                            'upload_npwp' => 'NPWP',
                            'upload_cv' => 'Curriculum Vitae (CV)',
                            'upload_sk_kerja' => 'SK Kerja',
                            'upload_keterangan_sehat' => 'Surat Keterangan Sehat',
                            'upload_pakta_integritas' => 'Pakta Integritas',
                            'upload_ijazah' => 'Ijazah Terakhir',
                            'upload_sertifikat_k3' => 'Sertifikat K3',
                        ];
                    @endphp
                    ...
                    @foreach ($fields as $field => $label)
                        <div x-data="{ fileName: '', filePreview: null }" class="space-y-3">
                            ...
                            @if ($dataTraining && $dataTraining->$field)
                                <div class="text-sm text-blue-500 dark:text-blue-300">
                                    <a href="{{ $dataTraining->{$field . '_url'} }}" target="_blank"
            ...
            <!-- Tombol Hapus -->
            @if ($dataTraining)
                <form id="deleteForm" action="{{ route('data_training.destroy', $dataTraining->id) }}" method="POST"
                    class="mt-4">
                    @csrf
                    @method('DELETE')
...
<script>
    ...
    function uploadFile(fieldName, file, callback) {
        ...
        axios.post("{{ route('data_training.store', $detailPesertaId) }}", formData, {
            headers: {
